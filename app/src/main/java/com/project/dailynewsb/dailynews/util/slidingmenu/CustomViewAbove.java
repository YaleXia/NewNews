package com.project.dailynewsb.dailynews.util.slidingmenu;
...
public class CustomViewAbove extends ViewGroup {
	...
	/**
	 * Determines speed during touch scrolling 速度追踪器
	 */
	...
	public interface OnPageChangeListener {
		...
		/**
		 * This method will be invoked（调用） when the current page is scrolled,
		 * either as part of a programmatically initiated smooth（光滑平稳） scroll or
		 * a user initiated touch scroll.
		 * 
		 * @param position
		 *            Position index of the first page currently being
		 *            displayed（显示）. Page position+1 will be visible if
